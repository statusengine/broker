...
class ObjectSerializer : public ISerializer<DataType> {
  public:
    explicit ObjectSerializer(INebmodule &neb)
        : neb(neb), buffer(nullptr), writer(nullptr) {}
  ...
  protected:
    ...
    inline void EndChildObject() { writer->EndObject(); }

    inline void InsertJsonObject(const char *name, std::string_view value) {
        writer->Key(name);
        writer->RawValue(value.data(), value.size(),
