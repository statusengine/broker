...
class MessageHandlerConfiguration {
  public:
    bool InitLoad(const toml::table &tbl) {
        for (auto &tableEntry : tbl) {
            auto qName = wise_enum::from_string<Queue>(
                std::string(tableEntry.first).c_str());
            if (qName.has_value()) {
                try {
                    (*queues)[qName.value()] =
                        toml::get<std::string>(tableEntry.second);
                } catch (const toml::type_error &tte) {
                    se.Log() << "Invalid configuration: Invalid value for key "
                             << tableEntry.first << LogLevel::Error;
                    return false;
                }
            } else {
                auto wqName = wise_enum::from_string<WorkerQueue>(
                    std::string(tableEntry.first).c_str());
                if (wqName.has_value()) {
                    try {
                        (*workerQueues)[wqName.value()] =
                            toml::get<std::string>(tableEntry.second);
                    } catch (const toml::type_error &tte) {
                        se.Log()
                            << "Invalid configuration: Invalid value for key "
                            << tableEntry.first << LogLevel::Error;
                        return false;
                    }
                }
    ...
    const std::shared_ptr<std::map<WorkerQueue, std::string>>
    GetWorkerQueueNames() const {
        return workerQueues;
    }
  ...
  protected:
    ...
    template <typename T>
    T GetTomlDefault(const toml::value &tab, const char *ky, T &&opt) const {
        try {
            return toml::find<T>(tab, ky);
            // return toml::find_or(tab, ky, opt);
        } catch (const toml::type_error &tte) {
            se.Log() << "Invalid configuration: Invalid value for key " << ky
                     << LogLevel::Error;
            return std::move(opt);
...
class RabbitmqConfiguration : public MessageHandlerConfiguration {
  public:
    explicit RabbitmqConfiguration(IStatusengine &se)
        : MessageHandlerConfiguration(se), Port(5673), Timeout(),
          Exchange("statusengine"), SSL(false), SSLVerify(true) {}
    ...
    bool Load(const toml::table &tbl) override {
        ...
        if (Hostname.empty()) {
            se.Log() << "Please specify a hostname in rabbitmq configuration"
                     << LogLevel::Error;
            return false;
        }
        ...
        Username =
            GetTomlDefault<>(tbl, "Username", std::string("statusengine"));
        ...
        Exchange =
            GetTomlDefault<>(tbl, "Exchange", std::string("statusengine"));
...
class GearmanConfiguration : public MessageHandlerConfiguration {
  public:
    explicit GearmanConfiguration(IStatusengine &se)
        : MessageHandlerConfiguration(se) {}

    bool Load(const toml::table &tbl) override {
        try {
            URL = toml::get<std::string>(tbl.at("URL"));
        } catch (const toml::type_error &tte) {
            se.Log() << "Invalid configuration: Invalid value for key URL"
                     << LogLevel::Error;
            return false;
        }
...
class Configuration {
  public:
    explicit Configuration(IStatusengine &se)
        : se(se), maxWorkerMessagesPerInterval(0), logLevel(LogLevel::Warning) {
    }
    ~Configuration() {
    ...
    bool Load(const std::string &configurationPath) {
        try {
            ...
            se.Log() << "Could not read file: " << rte.what()
                     << LogLevel::Error;
            return false;
        } catch (toml::syntax_error &ste) {
            se.Log() << "configuration syntax error: " << ste.what()
                     << LogLevel::Error;
            return false;
        }
        ...
        try {
            ...
            se.Log() << "Invalid configuration: Bulk isn't a table!"
                     << LogLevel::Error;
            return false;
        }
        ...
        try {
            ...
            auto logLevelStr =
                toml::find_or<std::string>(logTable, "Level", "Warning");
            if (logLevelStr == "Info") {
                ...
                se.Log() << "Invalid configuration: Unknown log level: "
                         << logLevelStr << LogLevel::Error;
                return false;
            }
        } catch (std::out_of_range &oor) {
        } catch (const toml::type_error &tte) {
            se.Log() << "Invalid configuration: Log isn't a table!"
                     << LogLevel::Error;
            return false;
        }
        ...
        try {
            std::vector<std::string> bulkQueueList =
                toml::get<std::vector<std::string>>(bulkTable.at("Queues"));
            for (auto &bulkQueueItem : bulkQueueList) {
                auto queue =
                    wise_enum::from_string<Queue>(bulkQueueItem.c_str());
                if (queue.has_value()) {
                    bulkQueues.insert(queue.value());
                } else {
                    se.Log() << "Invalid configuration: Bulk::Queues contains "
                                "an unknown queue identifier: "
            ...
            se.Log() << "Invalid configuration: Bulk::Queues isn't an array!"
                     << LogLevel::Error;
            return false;
        }
        ...
        try {
            ...
            se.Log() << "Invalid configuration: Scheduler isn't a table!"
                     << LogLevel::Error;
            return false;
        }

        try {
            std::vector<toml::table> gearmans =
                toml::get<std::vector<toml::table>>(cfg.at("Gearman"));
            ...
            se.Log()
                << "Invalid configuration: Gearman isn't an Array of Tables!"
                << LogLevel::Error;
            return false;
        }

        try {
            std::vector<toml::table> rabbits =
                toml::get<std::vector<toml::table>>(cfg.at("Rabbitmq"));
            ...
            se.Log()
                << "Invalid configuration: Rabbitmq isn't an Array of Tables!"
                << LogLevel::Error;
            return false;
        }

        try {
            maxWorkerMessagesPerInterval = toml::find_or<unsigned long>(
                cfg.at("Worker"), "MaxWorkerMessagesPerInterval", 1000000ul);
        } catch (const std::out_of_range &oor) {
        ...
        for (auto &gearmanClient : gearman) {
            ...
            for (auto &queueName : *queueNames) {
                se.Log() << "Gearman " << counter
                         << " queue name: " << queueName.second
                         << LogLevel::Info;
            }
        }
    ...
    time_t GetBulkFlushInterval() const {
        return GetTomlDefault<time_t>(bulkTable, "FlushInterval",
    ...
    std::vector<std::shared_ptr<GearmanConfiguration>> *
    GetGearmanConfiguration() {
        return &gearman;
    }

    std::vector<std::shared_ptr<RabbitmqConfiguration>> *
