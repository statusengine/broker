...
MessageHandlerList::MessageHandlerList(IStatusengine &se, Configuration &cfg)
    ...
    // flushInProgress is set to true to ensure no messages are sent until
    // initialization is complete

    maxBulkSize = cfg.GetBulkMaximum();
    std::map<Queue,
             std::shared_ptr<std::vector<std::shared_ptr<IMessageHandler>>>>
        handlers;
    auto InsertHandler = [&handlers](Queue queue,
        ...
        try {
            ...
            queueHandlers = std::make_shared<
                std::vector<std::shared_ptr<IMessageHandler>>>();
    ...
    for (auto &rabbitmqConfig : *rabbitmqConfigs) {
        auto rabbitmqClient =
            std::make_shared<RabbitmqClient>(se, rabbitmqConfig);
    ...
    for (auto &qHandlerPair : handlers) {
        mqHandlers[qHandlerPair.first] = std::make_shared<MessageQueueHandler>(
            se, *this, qHandlerPair.first, qHandlerPair.second,
            cfg.IsBulkQueue(qHandlerPair.first));
    }
}
