...
class GearmanWorkerContext {
  public:
    GearmanWorkerContext(WorkerQueue queue, GearmanClient *client)
        : Queue(queue), Client(client){};
    WorkerQueue Queue;
    GearmanClient *Client;
};

gearman_return_t se_gearman_worker_callback(gearman_job_st *job,
                                            void *worker_context) {
...
GearmanClient::GearmanClient(IStatusengine &se,
...
bool GearmanClient::Connect() {
    if (!queueNames->empty()) {
        ...
        if (gearman_success(ret)) {
            se.Log() << "Added gearman client server connection"
                     << LogLevel::Info;
        } else {
            se.Log() << "Could not add client gearman server: "
    ...
    if (!workerQueueNames->empty()) {
        ...
        if (gearman_success(ret)) {
            se.Log() << "Added gearman worker server connection"
                     << LogLevel::Info;
        } else {
            se.Log() << "Could not add worker gearman server: "
                     << gearman_client_error(client) << LogLevel::Error;
            return false;
        }
        for (auto &queue : *workerQueueNames) {
            ...
            ret = gearman_worker_define_function(worker, queue.second.c_str(),
                                                 queue.second.size(), cbfn, 0,
                                                 ctx);
            if (gearman_success(ret)) {
                se.Log() << "Added gearman worker function " << queue.second
                         << LogLevel::Info;
            } else {
                se.Log() << "Could not add gearman worker function "
                         << queue.second << ": " << gearman_client_error(client)
                         << LogLevel::Error;
                return false;
            }
        }
...
void GearmanClient::SendMessage(Queue queue, const std::string_view message) {
    ...
    auto ret =
        gearman_client_do_background(client, queueName.c_str(), nullptr,
                                     message.data(), message.length(), nullptr);
    if (!gearman_success(ret)) {
...
gearman_return_t GearmanClient::WorkerCallback(WorkerQueue queue,
                                               gearman_job_st *job) {
    std::string msg(static_cast<const char *>(gearman_job_workload(job)),
...
bool GearmanClient::Worker(unsigned long &counter) {
    ...
    if (!workerQueueNames->empty()) {
        ...
        case GEARMAN_NO_ACTIVE_FDS:
            se.Log() << "Gearman worker is not connected to server"
                     << LogLevel::Error;
            break;
        default:
            se.Log() << "Unknown gearman worker error: " << ret
                     << LogLevel::Error;
        }
    }
    return moreJobs;
