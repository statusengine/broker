...
class MessageHandlerList : public IMessageHandlerList,
                           public IBulkMessageCounter {
  public:
    MessageHandlerList(IStatusengine &se, Configuration &cfg);

    ~MessageHandlerList() override { FlushBulkQueue(); }

    void InitComplete() override { flushInProgress = false; }
    ...
    std::shared_ptr<IMessageQueueHandler>
    GetMessageQueueHandler(Queue queue) override {
