...
Nebmodule::Nebmodule(Neb_NebmodulePtr handle, std::string args)
    : handle(handle) {
    uc = uchardet_new();
...
bool Nebmodule::RegisterCallback(NEBCallbackType cbType) {
    ...
    if (result != 0) {
        se->Log() << "Could not register nebmodule_callback for Event Type "
                  << cbType << ": " << result << LogLevel::Error;
        return false;
    } else {
        se->Log() << "Register nebmodule_callback for Event Type: " << cbType
                  << LogLevel::Info;
        return true;
    }
}
...
std::string Nebmodule::EncodeString(char *inputData) {
    ...
    auto lendata =
        std::strlen(inputData); // we can't use strnlen here, we don't have any
                                // idea of the length here...
    ...
    char *outputData =
        new char[outputDataLength]; // utf-8 possibly needs up to 4 bytes for a
                                    // single character :/

    auto cd = iconv_open("UTF-8", charset);
    auto outputLength =
        iconv(cd, &inputData, &lendata, &outputData, &outputDataLength);
...
extern "C" int nebmodule_init(int, char *args, nebmodule *handle) {
    try {
        instance = std::make_shared<statusengine::Nebmodule>(handle,
                                                             std::string(args));
        instance->Init();
        return 0;
