...
class NebmoduleCallback {
  public:
    explicit NebmoduleCallback(NEBCallbackType cbType, IStatusengine &se)
        : se(se), cbType(cbType) {}
    ...
    virtual NEBCallbackType GetCallbackType() { return cbType; }
...
class StandardCallback : public NebmoduleCallback {
  public:
    ...
    StandardCallback(StandardCallback &&other) noexcept
        : NebmoduleCallback::NebmoduleCallback(std::move(other)),
          qHandler(std::move(other.qHandler)) {}
...
template <typename Nebtype, typename Serializer, NEBCallbackType CBT,
          Queue queue>
class StandardCallbackSerializer : public NebmoduleCallback {
  public:
    explicit StandardCallbackSerializer(IStatusengine &se)
        : NebmoduleCallback(CBT, se), serializer(se.GetNebmodule()) {
        qHandler = se.GetMessageHandler()->GetMessageQueueHandler(queue);
    }
    ...
    void Callback(int, void *data) override {
        qHandler->SendMessage(
            serializer.ToJson(*(reinterpret_cast<const Nebtype *>(data))));
    }
  ...
  public:
    explicit ServiceCheckCallback(IStatusengine &se)
        : NebmoduleCallback(NEBCALLBACK_SERVICE_CHECK_DATA, se),
          servicechecks(false), ocsp(false), service_perfdata(false),
          checkDataSerializer(se.GetNebmodule()),
          perfDataSerializer(se.GetNebmodule()) {
        auto mHandler = se.GetMessageHandler();
        if (mHandler->QueueExists(Queue::ServiceCheck)) {
            serviceCheckHandler =
                mHandler->GetMessageQueueHandler(Queue::ServiceCheck);
            servicechecks = true;
        }
        ...
        if (mHandler->QueueExists(Queue::ServicePerfData)) {
            servicePerfHandler =
                mHandler->GetMessageQueueHandler(Queue::ServicePerfData);
    ...
    ServiceCheckCallback(ServiceCheckCallback &&other) noexcept
        : NebmoduleCallback::NebmoduleCallback(std::move(other)),
          servicechecks(other.servicechecks), ocsp(other.ocsp),
          service_perfdata(other.service_perfdata),
          checkDataSerializer(std::move(other.checkDataSerializer)),
          perfDataSerializer(std::move(other.perfDataSerializer)),
          serviceCheckHandler(std::move(other.serviceCheckHandler)),
          ocspHandler(std::move(other.ocspHandler)),
    ...
    void Callback(int, void *vdata) override {
        ...
        if (data->type == NEBTYPE_SERVICECHECK_PROCESSED) {
            ...
            if (service_perfdata &&
                temp_service->process_performance_data != 0) {
                servicePerfHandler->SendMessage(
                    perfDataSerializer.ToJson(*data));
            }
        }
    }
...
class ProcessDataCallback : public NebmoduleCallback {
  public:
    explicit ProcessDataCallback(IStatusengine &se)
        : NebmoduleCallback(NEBCALLBACK_PROCESS_DATA, se), restartData(false),
          processData(false), startupSchedulerMax(se.GetStartupScheduleMax()),
          serializer(se.GetNebmodule()) {
        auto mHandler = se.GetMessageHandler();
        if (mHandler->QueueExists(Queue::RestartData)) {
            restartHandler =
                mHandler->GetMessageQueueHandler(Queue::RestartData);
            restartData = true;
        }
        if (mHandler->QueueExists(Queue::ProcessData)) {
            processHandler =
                mHandler->GetMessageQueueHandler(Queue::ProcessData);
            processData = true;
        }
    }
    ...
    ProcessDataCallback(ProcessDataCallback &&other) noexcept
        : NebmoduleCallback::NebmoduleCallback(std::move(other)),
          restartData(other.restartData), processData(other.processData),
          startupSchedulerMax(other.startupSchedulerMax),
          serializer(std::move(other.serializer)),
          restartHandler(std::move(other.restartHandler)),
    ...
    void Callback(int, void *vdata) override {
        ...
        if (startupSchedulerMax > 0 &&
            data->type == NEBTYPE_PROCESS_EVENTLOOPSTART) {
            se.Log() << "Reschedule all hosts and services" << LogLevel::Info;
            for (auto temp_host = host_list; temp_host != nullptr;
                 temp_host = temp_host->next) {
                auto now = std::time(nullptr);
                time_t check_interval =
                    static_cast<time_t>(temp_host->check_interval) *
                    interval_length;
                time_t delay;
                if ((now - temp_host->last_check) > check_interval) {
                    delay = ranged_urand(
                        0, static_cast<unsigned int>(
                               std::min(startupSchedulerMax, check_interval)));
                } else {
            ...
            for (auto temp_service = service_list; temp_service != nullptr;
                 temp_service = temp_service->next) {
                auto now = std::time(nullptr);
                time_t check_interval =
                    static_cast<time_t>(temp_service->check_interval) *
                    interval_length;
                time_t delay;
                ...
                se.GetNebmodule().ScheduleServiceCheckDelay(temp_service,
                                                            delay);
            }
...
class HostCheckCallback : public NebmoduleCallback {
  public:
    explicit HostCheckCallback(IStatusengine &se)
        : NebmoduleCallback(NEBCALLBACK_HOST_CHECK_DATA, se), hostchecks(false),
          ochp(false), serializer(se.GetNebmodule()) {
        auto mHandler = se.GetMessageHandler();
        if (mHandler->QueueExists(Queue::HostCheck)) {
            hostCheckHandler =
                mHandler->GetMessageQueueHandler(Queue::HostCheck);
            hostchecks = true;
        }
    ...
    HostCheckCallback(HostCheckCallback &&other) noexcept
        : NebmoduleCallback::NebmoduleCallback(std::move(other)),
          hostchecks(other.hostchecks), ochp(other.ochp),
          serializer(other.serializer),
          hostCheckHandler(std::move(other.hostCheckHandler)),
          ochpHandler(std::move(other.ochpHandler)) {}
